...
class TodoItemsRepository {
    companion object {
        @Volatile private var itemsList = arrayListOf<TodoItem>()
    }
    ...
    private suspend fun addToDAO(item: TodoItem) {
        coroutineScope {
            withContext(Dispatchers.IO) {
                MyApp.instance.todoItemDAO.insertItem(item)
            }
        }
    }
    ...
    private suspend fun updateItemInDAO(item: TodoItem) {
        coroutineScope {
            withContext(Dispatchers.IO) {
                MyApp.instance.todoItemDAO.updateItem(item)
            }
        }
    }
    ...
    private suspend fun deleteInDAO(item: TodoItem) {
        coroutineScope {
            withContext(Dispatchers.IO) {
                MyApp.instance.todoItemDAO.deleteItem(item)
            }
        }
    }
